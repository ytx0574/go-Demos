...
6. 结构体可增加方法, 为结构体增加一些扩展
	1. 方法的调用机制和原理:
		1. 方法的调用和函数基本一致, 不一样的地方. 方法在调用时, 会传入自己(类似oc的实例方法, 默认带self).
		2. 方法的调用带入的自己, 要区分是带入的引用类型还是值类型.  引用类型带入的是地址, 修改影响到其他地方. 值类型是直接拷贝数据, 可能影响性能
		3. go中的方法是指定数据类型的, 所以任何类型都可以添加方法. 类似oc的category扩展(系统类型无法直接扩展, 需要重定义一个type, 再进行扩展)
		4. go中可对任何类型实现String()方法, 返回该实例的描述, 此方法不能包含*. 比如Print的时候, 会调用它  (类似oc的description).
